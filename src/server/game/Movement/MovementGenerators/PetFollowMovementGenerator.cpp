...
PetFollowMovementGenerator::PetFollowMovementGenerator(Unit* target, float range, float angle) : AbstractFollower(ASSERT_NOTNULL(target)),
    _range(range), _angle(angle), _hasStopped(false) { }

PetFollowMovementGenerator::~PetFollowMovementGenerator() { }
...
void PetFollowMovementGenerator::Initialize(Unit* owner)
{
...
bool PetFollowMovementGenerator::Update(Unit* owner, uint32 diff)
{
...
void PetFollowMovementGenerator::Finalize(Unit* owner)
{
